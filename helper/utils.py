...
# --- QUEUE SYSTEM ADDITIONS ---
QUEUE = asyncio.Queue(maxsize=20)  # Queue with max size of 20
TASK_POSITIONS = {}  # Track task positions for each user
...
async def Compress_Stats(e, userid):
    if int(userid) not in [e.from_user.id, 0]:
        return await e.answer(f"⚠️ Hᴇʏ {e.from_user.first_name}\nYᴏᴜ ᴄᴀɴ'ᴛ sᴇᴇ sᴛᴀᴛᴜs ᴀs ᴛʜɪs ɪs ɴʏᴏᴜʀ ғɪʟᴇ", show_alert=True)
...
async def CompressVideo(bot, query, ffmpegcode, c_thumb):
    ...
    if QUEUE.full():
        return await ms.edit("⚠️ Queue is full (20 tasks limit). Please try again later.")

    # Add task to queue
    task = (bot, query, ffmpegcode, c_thumb, ms)
    await QUEUE.put(task)
    
    # Calculate task position
    task_position = QUEUE.qsize()
    TASK_POSITIONS[UID] = (task_position, ms)
    
    # Send initial queue message
    if task_position > 1:
        await ms.edit(f"Task added to the queue, please wait till other task completes\nYour task position: {task_position}")
        return
...
async def process_next_in_queue():
    global CURRENTLY_PROCESSING
    if not QUEUE.empty() and not CURRENTLY_PROCESSING:
        # Update positions for remaining tasks
        for uid in list(TASK_POSITIONS.keys()):
            pos, ms = TASK_POSITIONS[uid]
            new_pos = pos - 1
            if new_pos > 0:
                TASK_POSITIONS[uid] = (new_pos, ms)
                try:
                    await ms.edit(f"Task added to the queue, please wait till other task completes\nYour task position: {new_pos}")
                except:
                    pass
            else:
                del TASK_POSITIONS[uid]

        # Process next task
        task = await QUEUE.get()
        bot, query, ffmpegcode, c_thumb, ms = task
        CURRENTLY_PROCESSING = True
        try:
            await ms.edit('⚠️__**Please wait...**__\n**Starting your task now...**')
            await CompressVideo(bot, query, ffmpegcode, c_thumb)
        except Exception as e:
